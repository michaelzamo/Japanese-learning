      ...
      <main className="w-full max-w-7xl mx-auto overflow-hidden relative bg-gray-50">
        ...
        {currentView === 'reader' && (
          <div className="h-full w-full p-2 md:p-4">
             {!tokens.length ? (
              // MODE ÉDITEUR (On utilise une sous-grille pour titre + zone de texte)
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 h-full grid grid-rows-[auto_auto_1fr] p-6 gap-4">
                ...
                <input type="text" className="w-full p-4 text-xl font-bold border-2 border-gray-100 rounded-xl outline-none" placeholder="Titre..." value={title} onChange={(e) => setTitle(e.target.value)} />
                {/* overflow-auto sur le textarea pour qu'il scrolle DANS la grille */}
                <textarea className="w-full p-6 text-lg leading-loose border-2 border-gray-100 rounded-xl outline-none resize-none shadow-inner overflow-auto" placeholder="Collez le texte ici..." value={input} onChange={(e) => setInput(e.target.value)} />
              </div>
            ) : (
              // MODE LECTURE / SPLIT VIEW
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 h-full overflow-hidden grid grid-rows-[auto_1fr]">
                ...
                {/* Le Reader occupe la 2ème ligne de la grille */}
