...
# 2. Créer une nouvelle carte à réviser
@app.post("/cards")
def create_card(card: NewCard):
    db = SessionLocal()
    # On vérifie si le mot existe déjà pour éviter les doublons
    ...
    new_card = Card(
        ...
        next_review=datetime.utcnow() # Révision immédiate
    )
    db.add(new_card)
    db.commit()
    db.refresh(new_card)
...
# 3. Récupérer les mots à réviser aujourd'hui
@app.get("/reviews")
def get_reviews():
    db = SessionLocal()
    now = datetime.utcnow()
    # On cherche les cartes dont la date est passée ou égale à maintenant
    reviews = db.query(Card).filter(Card.next_review <= now).all()
    db.close()
    return reviews

# 4. Enregistrer le résultat d'une révision (L'ALGORITHME SRS)
@app.post("/review")
def submit_review(payload: ReviewPayload):
    ...
    # Algorithme simplifié inspiré de SM-2
